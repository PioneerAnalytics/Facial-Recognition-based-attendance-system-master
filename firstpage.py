...
mydb = mysql.connector.connect(host="localhost",user="root",passwd="2712root",database="student_database")
mycursor = mydb.cursor()

sql="UPDATE Attendance SET stats='Absent' WHERE stats='Present';"
...
def function5():
    root.destroy()
def function6():
    os.system("python EnterDetails.py")
...
Button(root,text="Enter students details",font=('times new roman',20),bg="black",fg="white",command=function6).grid(row=7,columnspan=2,sticky=N+E+W+S,padx=5,pady=5)
Button(root,text="Exit",font=('times new roman',20),bg="black",fg="white",command=function5).grid(row=8,columnspan=2,sticky=N+E+W+S,padx=5,pady=5)
